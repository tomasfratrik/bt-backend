...
def grisa():
    ...
    if request.method == 'POST':
        ...
        with open(img.get_absolute_path(), "rb") as f:
            files = {"file": f}
            res = requests.post(post_server_url, files=files)

        res_json = res.json()
        similar_images = res_json["similiar_img_json"]
        source_images = res_json["source_img_json"]
        output = (similar_images, source_images)
